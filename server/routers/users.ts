...
export const adminRouter = router({
  ...
  updateUserRole: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      // Notify the agent about the user status change
      if (isApprovedStatus) {
        default_api.message(type="info", text=`Novo usuário aprovado: ${userToUpdate.email}`);
      } else if (role === "rejected") {
        default_api.message(type="info", text=`Novo usuário rejeitado: ${userToUpdate.email}`);
      }
