    ...
    Alert.alert(
      ...
      [
        { text: "Cancelar", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              const result = await updateUserRoleMutation.mutateAsync({ userId, role: newRole });
              Alert.alert("Sucesso", "Função do usuário atualizada com sucesso!");
              if (result.newRole === "user" || result.newRole === "admin") {
                default_api.message(type="info", text=`Novo usuário aprovado: ${result.userEmail}`);
              } else if (result.newRole === "rejected") {
                default_api.message(type="info", text=`Novo usuário rejeitado: ${result.userEmail}`);
              }
